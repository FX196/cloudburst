...
TEST_F(SchedulerHandlerTest, FunctionCreateReceive){
    ...
    kvs_put(kvs_mock, FUNCOBJ, serialize(func_lattice), log_, LatticeType::SET);
    ...
    EXPECT_EQ(((KvsMockClient *) kvs_mock)->keys_put_.size(), 3);
    EXPECT_EQ(((KvsMockClient *) kvs_mock)->keys_put_[1], FUNC_PREFIX + "foo");
    EXPECT_EQ(((KvsMockClient *) kvs_mock)->keys_put_[2], FUNCOBJ);
